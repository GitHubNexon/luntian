...
const AccountsPayableChart = () => {
  ...
  const expenseNotes = [
    "• The Total Expense is based on payment dates, representing actual outflow of cash.",
    "• The Total Liabilities Amount is based on the bill dates, representing the total expenses billed during the period, including unpaid bills.",
    "• Note: The chart is disabled in the overall view to maintain consistency between billed dates and payment dates.",
  ];
  ...
  const calculateTotalBilledAmount = () => {
    const monthlyTotal = new Array(12).fill(0);
    allBills.forEach((bill) => {
      const billDate = new Date(bill.billDate.$date || bill.billDate);
      const monthIndex = billDate.getMonth();

      if (billDate.getFullYear() !== selectedYear && !isOverallView) return;
      if (
        timePeriod === "month" &&
        monthIndex !== selectedMonth &&
        !isOverallView
      ...
      monthlyTotal[monthIndex] += bill.totalAmount || 0;
    });
    return monthlyTotal;
  };
  ...
  const calculateOverallExpenses = () => {
    ...
    allBills.forEach((bill) => {
      if (bill.payment && Array.isArray(bill.payment)) {
        bill.payment.forEach((payment) => {
          overallExpenses += payment.amount || 0;
        });
      }
    });
  ...
  const calculateOverallBilledAmount = () => {
    let overallBilled = 0;
    allBills.forEach((bill) => {
      overallBilled += bill.totalAmount || 0;
    });
    return overallBilled;
  };

  const monthlyExpenses = isOverallView
    ? new Array(1).fill(calculateOverallExpenses())
    : calculateMonthlyExpenses();

  const monthlyTotal = isOverallView
    ? new Array(1).fill(calculateOverallBilledAmount())
  ...
  const chartData = {
    ...
    datasets: [
      ...
      {
        type: "line",
        label: "Total Liabilities Amount",
        data: monthlyTotal,
            ...
            <div className="flex flex-col">
              ...
              <span className="text-xl font-bold text-green-600">
                ...
                {numberToCurrencyString(
                  monthlyTotal.reduce((acc, val) => acc + val, 0)
                )}
              </span>
            </div>
            <div className="flex flex-col">
              ...
              <span className="text-xl font-bold text-green-600">
                PHP{" "}
                {numberToCurrencyString(
                  monthlyExpenses.reduce((acc, val) => acc + val, 0)
                )}
              </span>
            </div>
          </div>
